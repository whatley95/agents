...
- Define target platforms, minimum OS versions, and device-class constraints explicitly.
- Distinguish shared cross-platform logic from platform-specific implementations.
- Quantify performance targets (startup time, memory, battery, frame drops) and expected impact.
- Specify exact integration surfaces (permissions, native modules, background tasks, push channels).
- Include offline, retry, and sync-conflict behavior for data-critical features.
- Prioritize solutions that maintain native UX conventions on both iOS and Android.
